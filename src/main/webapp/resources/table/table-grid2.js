var NewtouchContext = {};
NewtouchContext.sleepTime = 500;

(function($) {
	/**
	 * 通过Params发送请求
	 * @param params 名页面上的值 格式："{_action:methodName,currentPageIndex:1,pageSize:10,id:2,name:'name'}"
	 * @param successCallback success时候回调函数
	 * @param failCallback error时候回调函数
	 */
	$.fn.search = function(params,successCallback,failCallback) {
		var tableId = $(this).attr("id");
		
		if(params){
			//currentPageIndex默认为1
			var currentPageIndex = params["currentPageIndex"];
			if(!currentPageIndex || currentPageIndex==null){
				params = $.extend(params, {currentPageIndex:1});
			}
			$(this).params(params);
		}
